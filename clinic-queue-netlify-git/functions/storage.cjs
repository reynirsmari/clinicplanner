...
// Works when Blobs is auto-configured (via connectLambda(event) in the handler)
// and also supports manual credentials via env vars as a fallback.
...
async function getStore(){
  ...
  try {
    // Preferred: environment is already set up by connectLambda(event) in the handler
    return blobs.getStore(STORE_NAME);
  } catch (e) {
    // Fallback for environments without auto-config:
    ...
    if (siteID && token) {
      return blobs.getStore(STORE_NAME, { siteID, token });
    }
    throw e;
  }
}
